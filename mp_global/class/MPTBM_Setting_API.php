	...
	if (!class_exists('MPTBM_Setting_API')) {
		class MPTBM_Setting_API {
			...
			function callback_text($args) {
				$value = MPTBM_Global_Function::get_settings($args['section'], $args['id'], $args['std']);
			...
			function callback_datepicker($args) {
				...
				$date = MPTBM_Global_Function::get_settings($args['section'], $args['id'], $args['std']);
			...
			function callback_file($args) {
				...
				<label>
					<input type="text" name="<?php echo esc_attr($name); ?>" class="formControl" value="<?php echo esc_attr($value); ?>" placeholder="<?php echo esc_attr($placeholder); ?>"/>
					<input type="button" class="button wpsa-browse" value="<?php echo esc_attr($label); ?>"/>
				</label>
				<?php
			}
			...
			function callback_pages($args) {
				$dropdown_args = array(
					'selected' => MPTBM_Global_Function::get_settings($args['section'], $args['id'], $args['std']),
					'name' => $args['section'] . '[' . $args['id'] . ']',
			...
			function show_navigation() {
				...
				if ($count > 1) {
					?>
					<ul class="tabLists nav d-flex flex-column ">
						<?php foreach ($this->settings_sections as $tab) { ?>
							<li class="nav-item" data-tabs-target="#<?php echo esc_attr($tab['id']); ?>">
