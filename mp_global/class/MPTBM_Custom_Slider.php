...
*/
	...
	if ( ! class_exists( 'MPTBM_Custom_Slider' ) ) {
		class MPTBM_Custom_Slider {
			...
			public function super_slider( $post_id = '', $meta_key = '' ) {
				$type      = MPTBM_Global_Function::get_slider_settings( 'slider_type', 'slider' );
			...
			public function slider( $post_id, $image_ids ) {
				if ( is_array( $image_ids ) && sizeof( $image_ids ) > 0 ) {
					$showcase_position = MPTBM_Global_Function::get_slider_settings( 'showcase_position', 'right' );
					$column_class      = $showcase_position == 'top' || $showcase_position == 'bottom' ? 'area_column' : '';
					$slider_style      = MPTBM_Global_Function::get_slider_settings( 'slider_style', 'style_1' );
					?>
					<div class="superSlider placeholder_area fdColumn">
						...
						<?php
							$slider_indicator = MPTBM_Global_Function::get_slider_settings( 'indicator_visible', 'on' );
							$icon             = MPTBM_Global_Function::get_slider_settings( 'indicator_type', 'icon' );
			...
			public function post_thumbnail( $image_id = '' ) {
				$thumbnail = MPTBM_Global_Function::get_image_url( '', $image_id );
				if ( $thumbnail ) {
					?>
			...
			public function slider_showcase( $image_ids ) {
				$showcase = MPTBM_Global_Function::get_slider_settings( 'showcase_visible', 'on' );
			...
			public function slider_popup( $post_id, $image_ids ) {
				if ( is_array( $image_ids ) && sizeof( $image_ids ) > 0 ) {
					$popup_icon_indicator = MPTBM_Global_Function::get_slider_settings( 'popup_icon_indicator', 'on' );
					?>
					<div class="sliderPopup" data-popup="superSlider">
						<div class="superSlider">
							...
							<div class="popupFooter">
								<?php
									$indicator = MPTBM_Global_Function::get_slider_settings( 'popup_image_indicator', 'on' );
									if ( $indicator == 'on' ) {
										$this->image_indicator( $image_ids );
									}
								?>
			...
			public function get_slider_ids( $post_id, $key ) {
				...
				$image_ids = MPTBM_Global_Function::get_post_info( $post_id, $key, array() );
				if ( $thumb_id ) {
