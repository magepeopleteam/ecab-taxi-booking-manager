/* global mptbmWcPaymentManager, jQuery */
( function ( $ ) {
	'use strict';

	var cfg = window.mptbmWcPaymentManager || {};
	var i18n = cfg.i18n || {};
	...
	$( function () {
		...
		if ( ! $manager.data( 'mptbm-relocated' ) ) {
			$manager.closest( 'tr' ).addClass( 'woocommerce-field' );
			$manager.data( 'mptbm-relocated', true );
		}

		// -----------------------------------------------------------
		// Expand / collapse a gateway's native settings form
		// -----------------------------------------------------------
		$manager.on( 'click', '.mptbm-gw-configure-btn', function () {
			var $card = $( this ).closest( '.mptbm-gw-card' );
			var $body = $card.find( '.mptbm-gw-body' );
			var open = $body.is( ':visible' );

			$manager.find( '.mptbm-gw-body' ).slideUp( 150 );
			$manager.find( '.mptbm-gw-configure-btn' ).text( i18n.configure || 'Configure' );
		...
		// -----------------------------------------------------------
		$manager.on( 'change', '.mptbm-gw-toggle-input', function () {
			var $input = $( this );
			...
			ajax( {
				action: 'mptbm_wc_toggle_gateway',
		...
		// (it lives inside the settings <form>; nested forms are invalid).
		// -----------------------------------------------------------
		$manager.on( 'click', '.mptbm-gw-save-btn', function ( e ) {
			e.preventDefault();

			var $btn = $( this );
			var $form = $btn.closest( '.mptbm-gw-form' );
			var $card = $btn.closest( '.mptbm-gw-card' );
			var gatewayId = $form.data( 'gateway-id' );
			var $status = $form.find( '.mptbm-gw-status' );

			// Native WC field names are woocommerce_{id}_{field}; submit as-is.
			var payload = { action: 'mptbm_wc_save_gateway', gateway_id: gatewayId };
			...
			ajax( payload )
				.done( function ( res ) {
					if ( res && res.success ) {
						...
						applyEnabledState( $card, res.data.enabled === 'yes' );
						$card.find( '.mptbm-gw-toggle-input' ).prop( 'checked', res.data.enabled === 'yes' );
						setTimeout( function () {
		...
		function applyEnabledState( $card, isOn ) {
			...
			$card.find( '.mptbm-gw-badge' ).text( isOn ? ( i18n.enabled || 'Enabled' ) : ( i18n.disabled || 'Disabled' ) );
		}
